...
/* A camada de rede é carregada SOB DEMANDA. Assim, se a biblioteca do servidor
   não carregar (offline na primeira abertura, CDN fora do ar), a fila local
   continua funcionando normalmente — ela só não esvazia. */
let _auth = null;
async function rede() {
  if (_auth) return _auth;
  try { _auth = await import("./auth.js"); } catch (e) { _auth = null; }
  return _auth;
}
...
const ENVIOS = {
  ...
  progresso: async (p) => (await rede()).sb.rpc("registrar_progresso", {
    p_occasion_id: p.occasion_id,
    p_status: p.status,
    p_pontos: p.pontos ?? 0,
    p_updated_at: p.updated_at,
  }),
  perfil: async (p) => (await rede()).sb.from("perfis").update(p.campos).eq("id", p.user_id),
  pontos: async (p) => (await rede()).sb.rpc("somar_pontos", { p_delta: p.delta, p_origem: p.origem }),
};
...
export async function sincronizar({ motivo = "manual" } = {}) {
  ...
  const a = await rede();
  if (!a || !a.CONFIGURADO) return { modoLocal: true };  // sem servidor: a fila espera
  if (!navigator.onLine) return { offline: true };

  const uid = await a.usuarioId();
  if (!uid) return { semUsuario: true };         // visitante: fica tudo local
  if (!(await a.sessaoValida())) {               // token vencido e sem renovar
