...
| O quê | Onde | Por quê |
|---|---|---|
| **Situações** (diálogos, frases, blocos, missões) | `conteudo/situacoes.js` | Fonte única. Editar aqui muda o app inteiro |
| **Camada de sotaque** (troca de palavra, som, expressão) | `conteudo/situacoes.js`, em `VARIANTES` | Fica colada à situação que ela modifica |
| **Sotaques** (traços, frases de fundamento) | `index.html`, em `R` | Vai para `conteudo/sotaques.js` quando crescer |
| **Áudios** | embutidos no `index.html` | Tocam offline, sem servidor |
| **Progresso do usuário** | IndexedDB + tabela `user_progress` | É o único dado que precisa de nuvem |
| **Índice das situações** | tabela `occasions` (opcional) | Só para relatório e para anunciar situação nova sem republicar |

**O conteúdo NÃO vai para o banco.** Ele viaja com o app: assim funciona offline desde a primeira
abertura, não gasta banda, e não tem sincronização de conteúdo para dar errado. O banco guarda
apenas o que é do usuário.

Para editar uma situação: abra `conteudo/situacoes.js`, mude o texto, gere o áudio das frases novas,
suba o arquivo e troque a versão no `sw.js`.
