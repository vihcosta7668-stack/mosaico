...
/* Também sob demanda: gravar e ler progresso local nunca pode depender da rede. */
let _auth = null;
async function rede() {
...
export async function sincronizarDoServidor({ completo = false } = {}) {
  const a = await rede();
  if (!a || !a.CONFIGURADO || !navigator.onLine) return { pulado: true };
  const uid = await a.usuarioId();
  if (!uid) return { pulado: true };

  const desde = completo ? null : await getMeta("ultimo_pull");
  let q = a.sb.from("user_progress")
